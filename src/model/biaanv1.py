...
class BIAANV1(nn.Module):
    
    def __init__(self, args):
        super(BIAANV1, self).__init__()
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            if self.scale == 2 or self.scale == 3:
                bfea2 = self.downconv1(F.interpolate(bfea1, scale_factor=1/self.scale, mode='nearest'))
                bfea2 = self.lrelu(self.downatt1(bfea2))
                bfea2 = self.lrelu(self.LRconv1(bfea2))
                ...
                bfea2 = self.downconv1(F.interpolate(bfea1, scale_factor=1/2, mode='nearest'))
                bfea2 = self.lrelu(self.downatt1(bfea2))
                bfea2 = self.lrelu(self.LRconv1(bfea2))
                bfea2 = self.downconv2(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))
                bfea2 = self.lrelu(self.downatt2(bfea2))
                bfea2 = self.lrelu(self.LRconv2(bfea2))
                ...
                #                 self.upconv2.weight.transpose(0, 1), self.bupconv2_bias, 1, 1)
