...
class FSRCNNACV5(nn.Module):
    ...
    def __init__(self, args):
        super(FSRCNNACV5, self).__init__()
        ...
        if (self.scale & (self.scale - 1)) == 0:  # 缩放因子等于 2^n
            for i in range(int(log(self.scale, 2))):  #  循环 n 次
                self.add_module(f'up{i}', acb.ACBlock(num_feat, num_feat, 3, 1, 1, deploy=use_inf))
        elif self.scale == 3:  # 缩放因子等于 3
            self.up = acb.ACBlock(num_feat, num_feat, 3, 1, 1, deploy=use_inf)
        else:
            # 报错，缩放因子不对
