...
class BIAAN(nn.Module):
    
    def __init__(self, args):
        super(BIAAN, self).__init__()
        
        in_nc  = 3
        out_nc = 3
        nf  = 40
        unf = 40 # version 3 & version 4:40; original:24
        nb  = 16
        scale = args.scale[0]
        ...
        self.bconv_last = nn.Conv2d(unf, in_nc, 3, 1, 1, bias=True)  # v1:nf; v2:unf
        ...
        #### back downsampling in v1
        # self.bupconv1_bias = nn.Parameter(torch.randn(nf,))
        ...
        if self.scale == 4:
            ...
            #### back downsampling in v1
            # self.bupconv2_bias = nn.Parameter(torch.randn(nf,))
            
        self.conv_last = nn.Conv2d(unf, out_nc, 3, 1, 1, bias=True)
        self.bconv_first = nn.Conv2d(out_nc, nf, 3, 1, 1, bias=True)  # v1:unf; v2:nf
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            btrunk = self.trunk_conv(self.AAB_trunk(bfea1))
            bfea2 = bfea1 + btrunk
            ######################
            # version 1: totally mirror of layers, with input channel nf
            # version 3: totally mirror of layers, with nf == unf == 40
            if self.scale == 2 or self.scale == 3:
                # bfea2 = self.lrelu(self.HRconv1(bfea1))
                # bfea2 = self.lrelu(self.att1(bfea2))
                # bfea2 = self.upconv1(F.interpolate(bfea2, scale_factor=1/self.scale, mode='nearest'))  # v3
                # bfea2 = F.conv2d(F.interpolate(bfea2, scale_factor=1/self.scale, mode='nearest'),
                #                 self.upconv1.weight.transpose(0, 1), self.bupconv1_bias, 1, 1)
                #### version 4
                bfea2 = self.upconv1(F.interpolate(bfea2, scale_factor=1 / self.scale, mode='nearest'))
                bfea2 = self.lrelu(self.att1(bfea2))
                bfea2 = self.lrelu(self.HRconv1(bfea2))
            elif self.scale == 4:
                # bfea2 = self.lrelu(self.HRconv2(bfea1))
                # bfea2 = self.lrelu(self.att2(bfea2))
                # bfea2 = self.upconv2(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))  # v3
                # # bfea2 = F.conv2d(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'),
                # #                 self.upconv1.weight.transpose(0, 1), self.bupconv2_bias, 1, 1)
                # bfea2 = self.lrelu(self.HRconv1(bfea2))
                # bfea2 = self.lrelu(self.att1(bfea2))
                # bfea2 = self.upconv1(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))  # v3
                # # bfea2 = F.conv2d(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'),
                # #                 self.upconv1.weight.transpose(0, 1), self.bupconv1_bias, 1, 1)
                #### version 4
                bfea2 = self.upconv1(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))
                bfea2 = self.lrelu(self.att1(bfea2))
                bfea2 = self.lrelu(self.HRconv1(bfea2))
                bfea2 = self.upconv2(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))
                bfea2 = self.lrelu(self.att2(bfea2))
                bfea2 = self.lrelu(self.HRconv2(bfea2))


            ######################
            # version 2: same main process, which is not mirror, with input channel unf

            # btrunk = self.trunk_conv(self.AAB_trunk(bfea1))
            # bfea2 = bfea1 + btrunk
            #
            # if self.scale == 2 or self.scale == 3:
            #     bfea3 = self.upconv1(F.interpolate(bfea2, scale_factor=1/self.scale, mode='nearest'))
            #     bfea3 = self.lrelu(self.att1(bfea3))
            #     bfea3 = self.lrelu(self.HRconv1(bfea3))
            # elif self.scale == 4:
            #     bfea3 = self.upconv1(F.interpolate(bfea2, scale_factor=1/2, mode='nearest'))
            #     bfea3 = self.lrelu(self.att1(bfea3))
            #     bfea3 = self.lrelu(self.HRconv1(bfea3))
            #     bfea3 = self.upconv2(F.interpolate(bfea3, scale_factor=1/2, mode='nearest'))
            #     bfea3 = self.lrelu(self.att2(bfea3))
            #     bfea3 = self.lrelu(self.HRconv2(bfea3))

            ################
            # output with features
            bout = self.bconv_last(bfea2)

            bILR = F.interpolate(y, scale_factor=1/self.scale, mode='bilinear', align_corners=False)
            bout = bout + bILR

            # return out, bout
            return out, bout, fea1, fea3, bfea1, bfea2  # version 4
