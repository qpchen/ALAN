...
class Attention(nn.Module):
    def __init__(self, d_model, dw_ker=5, dwd_ker=7, dwd_pad=9, dwd_dil=3, use_acb=True, use_dbb=False, deploy=False, acb_norm="batch", use_attn=True):
        ...
        self.spatial_gating_unit = DCA(channels=d_model)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, bb_norm="BN", use_acb=True, use_dbb=False, deploy=False, acb_norm="batch", use_sfb=False):
        super().__init__()
        # if acb_norm not in ["batch", "layer", "no", "v8old", "inst"]:
        #     raise NotImplementedError 
        # if bb_norm not in ["BN", "LN", "no"]:
        #     raise NotImplementedError 
        if use_sfb:
            self.proj = SFB(in_chans, embed_dim)
        else:
            if use_acb:
                if use_dbb:
...
class SFB(nn.Module):
    def __init__(self, in_channel, dim):
        ...
        self.frequency = FourierConv(in_channel, dim)
        self.conv_out = nn.Conv2d(dim*2, dim, 1)

    def forward(self, x):
        out = self.conv_spatial(x) + x
        out = torch.cat((out, self.frequency(x)), 1)

        return self.conv_out(out)

class DCANV2AA(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(num_stages):
            patch_embed = OverlapPatchEmbed(img_size=img_size, # if i == 0 else img_size // (2 ** (i + 1)),
                                            ...
                                            use_sfb=False if i == 0 else True)
