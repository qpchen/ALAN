...
class BIAANV9A(nn.Module):
    
    def __init__(self, args):
        super(BIAANV9A, self).__init__()
        ...
        #### downsampling
        self.bLRconv1 = nn.Conv2d(unf, unf, 3, 1, 1, bias=True)
        self.batt1 = PA(unf)
        self.bdownconv1 = nn.Conv2d(unf, nf, 3, 1, 1, bias=True)

        if self.scale == 4:
            self.bLRconv2 = nn.Conv2d(unf, unf, 3, 1, 1, bias=True)
            self.batt2 = PA(unf)
            self.bdownconv2 = nn.Conv2d(unf, unf, 3, 1, 1, bias=True)
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            ##############################
            # train from HR to LR
            # y = y - F.interpolate(x, scale_factor=self.scale, mode='bilinear', align_corners=False)  # v7
            bfea1 = self.bconv_first(y)

            if self.scale == 2 or self.scale == 3:
                bfea2 = self.lrelu(self.bLRconv1(bfea1))
                bfea2 = self.lrelu(self.batt1(bfea2))
                bfea2 = self.bdownconv1(F.interpolate(bfea2, scale_factor=1/self.scale, mode='nearest'))
            elif self.scale == 4:
                bfea2 = self.lrelu(self.bLRconv2(bfea1))
            ...
            bILR = F.interpolate(y, scale_factor=1/self.scale, mode='bilinear', align_corners=False)  # v7
            bout = bout + bILR

            # return out, bout
            return out, bout, fea1, fea2, bfea2, bfea3
