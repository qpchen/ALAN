...
def make_back_layer(seq_layers, n_layers):
    layers = []
    for li in range(n_layers-1,-1,-1):
        layers.append(seq_layers[li])
    return nn.Sequential(*layers)
...
class BIAANV11(nn.Module):
    
    def __init__(self, args):
        super(BIAANV11, self).__init__()
        ...
        self.bAAB_trunk = make_back_layer(self.AAB_trunk, nb)
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            btrunk = self.bAAB_trunk(self.trunk_conv(bfea2))
            # btrunk = self.trunk_conv(self.bAAB_trunk(bfea2))
            bfea3 = bfea2 + btrunk
